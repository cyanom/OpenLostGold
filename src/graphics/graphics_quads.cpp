...
void GraphicsQuads::addRect(float pos[2], float size[2], float color[4]) {
    glm::vec2 p1 = glm::vec2(pos[0], pos[1]);
    glm::vec2 p2 = glm::vec2(pos[0], pos[1] + size[1]);
    glm::vec2 p3 = glm::vec2(pos[0] + size[0], pos[1] + size[1]);
    glm::vec2 p4 = glm::vec2(pos[0] + size[0], pos[1]);
...
void GraphicsQuads::updateView(glm::mat4 vp) {
...
void GraphicsQuads::updateQuads() {
    void* data;
    // VERTICES
...
void GraphicsQuads::setupVertexBuffer() {
    gb->createBuffer(nbQuads * 4 * sizeof(Vertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vertexBuffer, vertexMemory);
}

void GraphicsQuads::setupIndexBuffer() {
    gb->createBuffer(nbQuads * 6 * sizeof(uint16_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, indexBuffer, indexMemory);
}
...
void GraphicsQuads::setupPipeline() {
    // Shaders
    VkShaderModule vertShaderModule = gb->createShaderModule("res/shaders/graphics_quads/vert.spv");
    VkShaderModule fragShaderModule = gb->createShaderModule("res/shaders/graphics_quads/frag.spv");
